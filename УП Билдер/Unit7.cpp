...
void __fastcall TAdd::Image2Click(TObject *Sender)
{
	if(Edit1->Text!="" && Edit2->Text!="" && Memo1->Lines->Count > 0 && Memo2->Lines->Count > 0)
	{
		...
		Form4->ADOQuery1->SQL->Text= "SELECT * FROM Изделия";
		...
		for(int i=0;i<Memo1->Lines->Count;i++)
		{
			...
			Form4->ADOQuery1->FieldByName("Код Заказа")->AsInteger = StrToInt(Edit2->Text);
			Form4->ADOQuery1->FieldByName("Наименование изделия")->AsString = Memo1->Lines->Strings[i];
			Form4->ADOQuery1->FieldByName("Количество изделий")->AsInteger = StrToInt(Memo2->Lines->Strings[i]);
		...
		Form4->ADOQuery1->SQL->Text= "SELECT * FROM Заказ";
		Form4->ADOQuery1->Active = false;
		Form4->ADOQuery1->Active = true;
		...
		while(!Form4->ADOQuery1->Eof)
		{
			...
			if(Form4->ADOQuery1->FieldByName("Код Заказа")->AsInteger == StrToInt(Edit2->Text))
			{
				y=false;
				break;
			}
		...
		{
			Form4->ADOQuery1->Insert();
			Form4->ADOQuery1->FieldByName("Имя заказчика")->AsString = Edit1->Text;
			Form4->ADOQuery1->FieldByName("Код Заказа")->AsInteger = StrToInt(Edit2->Text);
			Form4->ADOQuery1->Post();
		...
		{
			ShowMessage("Заказ с таким кодом уже есть!");
        }
	...
	{
        ShowMessage("Заполнены не все данные!");
    }
...
{
	Form4->ADOQuery1->SQL->Text = "SELECT * FROM [Имена_Изделий]";
	Form4->ADOQuery1->Active = false;
	Form4->ADOQuery1->Active = true;
	Form4->ADOQuery1->First();
	while(!Form4->ADOQuery1->Eof)
	{
		ComboBox1->Items->Add(Form4->ADOQuery1->FieldByName("Наименование")->AsString);
		Form4->ADOQuery1->Next();
	}
}
//---------------------------------------------------------------------------
...
{
	 Image2->Picture->LoadFromFile("кнопка.png");
}
//---------------------------------------------------------------------------
...
{
	Image2->Picture->LoadFromFile("кнопка добавить опасити.png");
}
//---------------------------------------------------------------------------
