...
void __fastcall TChange::Image2Click(TObject *Sender)
{
		...
		if (Edit1->Text!="")
		{
			...
			Form4->ADOQuery1->SQL->Add("DELETE * FROM Изделия where [Код Заказа]="+Edit1->Text);
			Form4->ADOQuery1->ExecSQL();
			//Form4->ADOQuery1->First();
			/*while(!Form4->ADOQuery1->Eof)
			{
				if(Form4->ADOQuery1->FieldByName("Код Заказа")->AsInteger == StrToInt(Edit1->Text))
				{
		...
		/*Form4->ADOQuery1->Close();
		Form4->ADOQuery1->SQL->Clear();
		Form4->ADOQuery1->SQL->Add("Delete Изделия where [Код Товара] = :P1");
		Form4->ADOQuery1->Parameters->ParamByName("P1")->Value = Edit1->Text;
		Form4->ADOQuery1->ExecSQL();
//        OtdelADOQuery1->Close();  - лишнее
		Form4->ADOQuery1->SQL->Clear();
		Form4->ADOQuery1->SQL->Add("select *");
		Form4->ADOQuery1->SQL->Add("from Изделия");
		Form4->ADOQuery1->Open();    */
			   //Form4->ADOQuery1->SQL->Text = "DELETE FROM [Изделия] WHERE [Код Заказа]="+Edit1->Text;
			...
			Form4->ADOQuery1->Active = true;
			 ...
			 if(Form4->ADOQuery1->Locate("Код Заказа",Edit1->Text, Options))
			 {
			  Form4->ADOQuery1->Delete();
		...
		{
			ShowMessage("Введите код заказа!");
		}
}
//---------------------------------------------------------------------------
...
{
   Image2->Picture->LoadFromFile("применить опасити.png");
}
//---------------------------------------------------------------------------

void __fastcall TChange::Image2MouseLeave(TObject *Sender)
{
	Image2->Picture->LoadFromFile("применить.png");
}
//---------------------------------------------------------------------------
