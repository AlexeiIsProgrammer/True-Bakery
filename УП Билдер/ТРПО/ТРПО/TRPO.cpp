...
void __fastcall TForm15::InAccClick(TObject *Sender)
{
	if(login->Text=="" || pass->Text=="")
	 ...
	 else
	 {
		  ...
		  while(!ADOQuery1->Eof)
		  {
			   if(ADOQuery1->FieldByName("Логин")->AsString==logg)
			   {
				flag=true;
				break;
			   }
		  ...
		  {
			   if(ADOQuery1->FieldByName("Пароль")->AsString==wordd)
			   {
				Form15->Hide();
				Form1->Show();
			   }
...
void __fastcall TForm15::RegAccClick(TObject *Sender)
{
	...
	else
	{
		...
		if (wordd != password_1) {
			...
			{
				ADOQuery1->Insert();
				ADOQuery1->FieldByName("Логин")->AsString = logg;
				ADOQuery1->FieldByName("Пароль")->AsString = wordd;
				ADOQuery1->FieldByName("Идентификатор")->AsInteger = 0;
				ADOQuery1->Post();
				ADOQuery1->Last();
